...
const handleInput = (event: Event, index: number) => {
    ...
    if (value.length > 0) {
        ...
        if (index < props.length - 1) { // jump to next input
...
const handleKeyDown = (event: KeyboardEvent, index: number) => {
    ...
    if (event.key === 'Backspace') { // handle backspace (delete)
    ...
    if (event.key === 'ArrowLeft' && index > 0) { // handle arrow left
        event.preventDefault();
        inputRefs.value[index - 1]?.focus();
    }

    if (event.key === 'ArrowRight' && index < props.length - 1) { // handle arrow right
        event.preventDefault();
        inputRefs.value[index + 1]?.focus();
    }
